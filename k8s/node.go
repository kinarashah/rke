	...
	for retries := 0; retries < MaxRetries; retries++ {
		...
		for _, node := range nodes.Items {
			...
			if cloudProviderName == ExternalAWSCloudProviderName {
				//logrus.Infof("Checking hostname address for node [%v], cloud provider: %v", nodeName, cloudProviderName)
				for _, addr := range node.Status.Addresses {
					if addr.Type == v1.NodeInternalIP && nodeAddress == addr.Address {
						logrus.Infof("Found node [%s]: %v", nodeName, nodeAddress)
						return &node, nil
					}
				}
			}
...
func SyncNodeLabels(node *v1.Node, toAddLabels, toDelLabels map[string]string) {
	...
	logrus.Infof("oldLabels %#v", oldLabels)

	logrus.Infof("deleteLabels %#v", toDelLabels)
	// Delete Labels
	...
	logrus.Infof("toAddLabels %#v", toAddLabels)
	// ADD Labels
	for key, value := range toAddLabels {
		node.Labels[key] = value
	}

	logrus.Infof("nodeLabels net %#v", node.Labels)

	//logrus.Infof("NET LABELS oldlabels %#v, newLabels %#v", oldLabels, node.Labels)
}
